...
class CalendarCardPlugin(BasePlugin):
    def generate_image(self, settings, device_config):
        ...
        draw.text((cx - w // 2, month_y), month, fill=(145, 145, 145), font=font_month)
        y += h_month + gap1

        # Weekday (black)
        w, _ = text_size(weekday, font_weekday)
        weekday_y = y - int(base * 0.015)
        draw.text((cx - w // 2, weekday_y), weekday, fill=(0, 0, 0), font=font_weekday)
        y += h_weekday + gap2

        # Day (red)
        w, _ = text_size(day, font_day)
        day_y = y - int(base * 0.01)
        draw.text((cx - w // 2, day_y), day, fill=(255, 0, 0), font=font_day)
